...
l_bool  drag_context_ok = 0;
/* enable accelaration of moving */
l_bool  accel_moving_ok = 0;
...
{
  info_board_fcolor = color_get_from_ini("info_board_fcolor");
  info_board_b1color = color_get_from_ini("info_board_bcolor");
  info_board_b2color = color_get_from_ini("info_board_bcolor_gradient");
  info_board_key = (l_int)getininum_fromfile( INI_MAINFILE, "speed_keys", "info_board");
...
void  view_ini ( void )
{
   ...
   drag_context_ok = getininum_fromfile(INI_MAINFILE, INI_OPTIMALIZATION, "move_window_context");
   accel_moving_ok = getininum_fromfile(INI_MAINFILE, INI_OPTIMALIZATION, "enable_move_acceleration");
};
  ...
  if ( event->type & EV_MOUSE ) {
     ...
     if ( v && IS_OKTOSUBPROCESS(v) ) {
       ...
       if ( keyb->shifts & KB_CTRL_FLAG
            && OBJECT(mouse)->state & MO_SF_MOUSELDOWN ) {  /* drag data */
...
p_data  view_drag_data ( p_view o )
{
  ...
  if ( OBJECT(o)->get_data(OBJECT(o), &clipboard) ) {
    ...
    while ( OBJECT(mouse)->state & MO_SF_MOUSELPRESS ) { /* repeat until mouse is unpressed */
  ...
  if ( event->type & EV_MOUSE && owner )  {
    ...
    if ( !(_mode & DM_DRAGCONTEXT) )
      ...
      if ( _mode & DM_DRAGMOVE )
         ...
         mouse->set_mode(mouse, MO_MO_RECT, COLOR(CO_LIGHTGRAY), o->get_global_bounds(o, o->get_local_extent(o)));
      ...
      if ( _mode & DM_DRAGGROW ) {
         ...
         mouse->set_mode(mouse, MO_MO_GROW, COLOR(CO_LIGHTGRAY), o->get_global_bounds(o, o->get_local_extent(o)));
      };


    mouse->set_range(mouse, b);

    if ( _mode & DM_DRAGCONTEXT ) /* drag by context */

          mouse->set_mode(mouse, 0, 0, rect_empty);
...
static void blit_alpha ( BITMAP *s, BITMAP *d, int sx, int sy, int dx, int dy, int w, int h )
{
   int x = 0;
   int y = 0;
   ...
   for ( y = 0; y < h; y++ )
      for ( x = 0; x < w; x++ ) {
            int c = getpixel(s, x+sx, y+sy);
            int cc = getpixel(d, x+dx, y+dy);
            ...
            r = (int)(0.5*(double)(r)+0.5*(double)(rr));
            g = (int)(0.5*(double)(g)+0.5*(double)(gg));
            b = (int)(0.5*(double)(b)+0.5*(double)(bb));

            putpixel(d, x+dx, y+dy, makecol(r, g, b));
      };
};
...
//        blit_alpha(buffer, out, d.a.x, d.a.y, d.a.x, d.a.y, rect_sizex(d)+1, rect_sizey(d)+1);
...
void  view_move_view ( p_view o, t_rect r )
{
  ...
  if ( !rect_equals(r, safebounds) &&
      ...
      if ( !o->is_top_view(o) || !OBJECT(o)->owner_view(OBJECT(o)) ||
            o->is_draw_mode(o, DWM_CANTACCELMOVE) )
                                ...
                                t_rect **rout, l_bool sub, l_bool ws )
{
  ...
  while ( p ) {
    ...
    if ( !(state & OB_SF_VISIBLE) ||
    /* !(state & OB_SF_EXPOSED) ||*/
         (VIEW(p)->draw_mode & DWM_ICONICDRAW) ||
