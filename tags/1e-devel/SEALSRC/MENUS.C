...
l_color  pal_menu[] = {CO_SYSx20, CO_SYSx20, CO_BLACK, CO_BLUE, CO_WHITE, CO_SYSx17, CO_NOCOLOR};
l_color  pal_hormenu[] = {CO_SYSx20, CO_SYSx20, CO_BLACK, CO_BLUE, CO_WHITE, CO_SYSx17, CO_NOCOLOR};
...
void  menuview_draw_under_rect ( p_view o, t_rect r )
{
  ...
  if ( MENUVIEW(o)->safe_desktop ) {
    ...
    r = o->get_global_bounds(o, o->get_local_extent(o));

    BLIT(MENUVIEW(o)->safe_desktop, screen, 0, 0, r.a.x, r.a.y,
                                    CONTEXT_WIDTH(MENUVIEW(o)->safe_desktop),
                                    CONTEXT_HEIGHT(MENUVIEW(o)->safe_desktop));
...
void  menuview_draw ( p_view o )
{
  ...
  if ( out ) {
    ...
    button(out, r.a.x+p.x, r.a.y+p.y, r.b.x+p.x, r.b.y+p.y, COLOR(CO_BLACK), COLOR(CO_BLACK));
    button(out, r.a.x+p.x+1, r.a.y+p.y+1, r.b.x+p.x-1, r.b.y+p.y-1, COLOR(CO_WHITE), COLOR(CO_SYSx19));
...
void  menuview_save_desktop ( p_menuview o )
{
  ...
  if ( MENUVIEW(o)->safe_desktop ) /* if exist, destroy it */

      destroy_bitmap(MENUVIEW(o)->safe_desktop);

  /* create new bitmap */

  MENUVIEW(o)->safe_desktop = create_bitmap(rect_sizex(r)+1, rect_sizey(r)+1);


  if ( MENUVIEW(o)->safe_desktop ) /* blit screen, if there was enough memory */
...
void  menuview_draw_item ( p_menuview o, p_menuitem item )
{
  ...
  if ( i ) {
    ...
    if ( !rect_check_empty(r) ) {
       ...
       t_rect safe = rect_assign(r.a.x-1, r.a.y, r.b.x+1, r.b.y);
       ...
       if ( out ) {
          ...
          vline(out, r.a.x+p.x-1, r.a.y+p.y, r.b.y+p.y, COLOR(CO_WHITE));
          vline(out, r.b.x+p.x+1, r.a.y+p.y, r.b.y+p.y, COLOR(CO_SYSx19));
          ...
          if ( !i->name ) { /* draw LINE */
            ...
            button(out, r.a.x+p.x-1, r.a.y+p.y+dy, r.b.x+p.x+1, r.a.y+p.y+dy+1, COLOR(CO_BLACK), COLOR(CO_WHITE));
            ...
            { /* selected item */
               ...
               rect(out, r.a.x+p.x-1, r.a.y+p.y, r.b.x+p.x+1, r.b.y+p.y, COLOR(CO_BLUE));

               rectfill(out, r.a.x+p.x, r.a.y+p.y, r.b.x+p.x, r.b.y+p.y, vo->get_color(vo, 3));

            };

            if ( !i->enable ) /* disable item */

               fcolor = vo->get_color(vo, 5);
...
t_rect  hormenu_get_item_box_ex ( p_menuview o, p_menuitem item, t_rect r )
{
  ...
  if ( i )
    ...
    if ( i->name )
       ...
       return rect_assign(r.a.x+dx, r.a.y, r.a.x+FONT_GETSTRWIDTH(i->font, i->name, -1)+
                                                 FONT_GETSTRWIDTH(i->font, i->param, -1)+
                                                 STANDARD_MENUDIFF_SIZE+
                                                 icon_size(o)+5+dx+wx, r.b.y);

    else /* return line */

       return rect_assign(r.a.x+dx, r.a.y, r.a.x+dx+STANDARD_MENULINE_WIDTH+wx, r.b.y);
...
void  hormenu_draw_item ( p_menuview o, p_menuitem item )
{
  ...
  if ( i ) {
    ...
    if ( !rect_check_empty(r) ) {
       ...
       safe = rect_assign(r.a.x, r.a.y-1, r.b.x, r.b.y+1);
...
lib_begin ( void ) {
  ...
  if ( ap_process == AP_INIT ) {
          ...
          standard_menuitem_font = get_font_in_size ( "Helv", 15, 15 );

          if ( !standard_menuitem_font )

             standard_menuitem_font = font_system_bd;

          pal_menu[0] = color_get_from_ini("menu_background");
          pal_menu[1] = color_get_from_ini("menu_background_gradient");
          pal_menu[2] = pal_hormenu[2] = color_get_from_ini("menu_pasive_caption");
          pal_menu[3] = pal_hormenu[3] = color_get_from_ini("menu_active_title");
          pal_menu[4] = pal_hormenu[4] = color_get_from_ini("menu_active_caption");
          pal_menu[5] = pal_hormenu[5] = color_get_from_ini("menu_disable_caption");

          pal_hormenu[0] = color_get_from_ini("menu_horizontal_background");
          pal_hormenu[1] = color_get_from_ini("menu_horizontal_background_gradient");
  }
